...
function setFileName(fileID)
{
    ...
    $('#fileTitle' + fileID).html("<i class='icon icon-file-text'></i> &nbsp;" + fileName + '.' + extension);
    $('#copyFile' + fileID).val(fileName + '.' + extension);
    $('.renameFile' + fileID).closest('li').removeClass('hidden');
    $('#renameBox' + fileID).closest('li').addClass('hidden');
}
</script>
    <ul class="files-list">
      <?php foreach($files as $file):?>
          ...
          echo html::hidden("copyFile$file->id", $file->title);
          ...
          echo html::a('###', "<i class='icon icon-pencil-alt'></i>", '', "class='renameFile$file->id fileAction btn btn-link edit text-primary' onclick='showRenameBox($file->id)' title='{$lang->file->edit}'");
          echo html::a('###', "<i class='icon icon-trash'></i>", '', "class='fileAction btn btn-link text-primary' onclick='deleteFile($file->id, this)' title='$lang->delete'");
          echo '</span>';
          echo '</li>';?>
