    ...
    <form class="main-table table-task skip-iframe-modal not-watch" method="post" id='executionTaskForm'>
      ...
      <div class="table-footer">
        ...
        <div class="table-actions btn-toolbar">
          ...
          <?php if(common::hasPriv('task', 'batchChangeExecution')):?>
          <div class='btn-group dropup'>
            <?php
            $actionLink = $this->createLink('task', 'batchChangeExecution', '', '', true);
            echo html::a($actionLink, "<span title=''>{$lang->task->changeProject}</span>", '', "class='btn iframe' data-toggle='modal' onclick='return batchChangeExecutionClick(this)'");
            ?>
          </div>
          <?php endif;?>
...
<script>
function batchChangeExecutionClick(link)
{
    var taskIdList = [];
    var element = zui.DTable.query('#taskList').$;
    var checkedIDList = element.getChecks();
    $.each(checkedIDList, function(index, id) {taskIdList.push(id);});

    $.cookie('batchChangeTaskIdList', taskIdList.join(','), {expires:config.cookieLife, path:config.webRoot});
    $(link).attr('href');
