...
class myexecution extends execution
{
     ...
     * @return void
     */
    public function maintainRelation($executionID = 0, $chprojectID = 0)
    {
        if(!empty($_POST))
        {
            $this->execution->editRelationOfTasks($executionID);
            if(dao::isError()) die(js::error(dao::getError()));
            $locate = $this->app->tab == 'chteam' ? inlink('relation', "executionID=$executionID&chprojectID=$chprojectID") : inlink('relation', "executionID=$executionID");
