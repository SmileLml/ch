...
<div id="mainMenu" class="clearfix table-row">
  <div class="btn-toolbar pull-left">
    ...
    <div class='btn-group'>
      ...
      <ul class='dropdown-menu' style='max-height:240px; max-width: 300px; overflow-y:auto'>
        <?php
          ...
          echo "<li $class>" . html::a($this->createLink('chproject', 'grouptask', "projectID=$projectID&groupBy=$groupBy&filter=$filterKey&intanceProjectID=0"), $lang->chproject->allProject) . "</li>";
          foreach($intanceProjectPairs as $key => $intanceProjectName)
          {
              $class = $intanceProjectID == $key ? 'class="active"' : '';
              echo "<li $class>" . html::a($this->createLink('chproject', 'grouptask', "projectID=$projectID&groupBy=$groupBy&filter=$filterKey&intanceProjectID=$key", ""), $intanceProjectName, '', "title='{$intanceProjectName}' class='text-ellipsis'") . "</li>";
          }
        ?>
      </ul>
    </div>
    <?php if(isset($lang->chproject->groupFilter[$groupBy])):?>
    <?php foreach($lang->chproject->groupFilter[$groupBy] as $filterKey => $name):?>
    <?php
    $active = '';
    ...
    <?php endforeach;?>
    <?php else:?>
    <?php echo html::a(inlink('grouptask', "projectID=$projectID&groupBy=$groupBy"), "<span class='text'>{$lang->all}</span> <span class='label label-light label-badge'>{$allCount}</span>", '', "class='btn btn-link btn-active-text'");?>
    <?php endif;?>
  </div>
  <div class="btn-toolbar pull-right">
    <div class="btn-group">
      ...
      <ul class="dropdown-menu">
        ...
        $link  = common::hasPriv('task', 'export') ? $this->createLink('task', 'export', "execution=$projectID&orderBy=$orderBy&type=$browseType") : '#';
    ...
    $link = $this->createLink('task', 'create', "execution=0&storyID=0&moduleID=0&taskID=0&todoID=0&extra=&bugID=0&chprojectID=$projectID") . '#app=chteam';
...
<div id='tasksTable' class='main-table' data-ride='table' data-checkable='false' data-group='true' data-hot='true'>
  ...
  <div class="table-empty-tip">
    <p>
      ...
      <?php echo html::a($this->createLink('task', 'create', "execution=0&storyID=0&moduleID=0&taskID=0&todoID=0&extra=&bugID=0&chprojectID=$projectID") . '#app=chteam', "<i class='icon icon-plus'></i> " . $lang->task->create, '', "class='btn btn-info'");?>
      <?php endif;?>
    </p>
  </div>
  <?php else:?>
  <table class="table table-grouped">
    <thead>
      <tr class="<?php if($allCount) echo 'divider';?>">
        <th class="c-side text-left has-btn group-menu">
          <div class="dropdown">
            ...
            <ul class="dropdown-menu">
              ...
              if($project->lifetime == 'ops' && $key == 'story') continue;
              $active = $key == $groupBy ? "class='active'" : '';
              echo "<li $active>"; common::printLink('chproject', 'groupTask', "execution=$projectID&groupBy=$key", $value); echo '</li>';
              ?>
              <?php endforeach;?>
            </ul>
          </div>
        ...
        <th class="c-name text-left"><?php echo $lang->task->project;?></th>
    ...
    <tbody>
      ...
      <tr data-id='<?php echo $groupIndex?>' <?php if($groupIndex > 1 and $i == 0) echo "class='divider-top'";?>>
        ...
        <td class="c-name"><?php echo zget($intanceProjectPairs, $task->project);?></td>
        ...
        <td class="c-actions">
          <?php if(common::canModify('execution', $project)):?>
