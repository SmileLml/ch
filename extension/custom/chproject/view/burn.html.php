...
<?php js::set('executionName', $executionName);?>
...
<?php js::set('burnBy', $burnBy);?>
<?php js::set('zentaoProject', $zentaoProject);?>
<div id='mainMenu' class='clearfix'>
    <div class='btn-toolbar pull-left'>
        ...
        echo html::a('#', $lang->chproject->$weekend, '', "class='btn btn-link' id='weekend'");
        if(strpos($type, 'delay') !== false) echo html::a('#', $lang->chproject->$delay, '', "class='btn btn-link' id='delay'");

        if(common::canModify('execution', $execution)) common::printLink('execution', 'fixFirst', "execution=$executionID", $lang->execution->fixFirst, '', "class='btn btn-link iframe' data-width='700'");
        ...
        <?php endif;?>
        <div class='input-control w-150px'>
            <?php  echo html::select('projectBy', $intanceProjects, $zentaoProject, "class='form-control chosen'");?>
        </div>
    </div>
...
<script>
    ...
    {
        $.cookie('chburnBy', $(this).val(), {expires:config.cookieLife, path:config.webRoot});
        var interval = typeof($('#interval').val()) == 'undefined' ? 0 : $('#interval').val() ;
        location.href = createLink('chproject', 'burn', 'projectID=' + projectID + '&type=' + type + '&interval=' + interval + '&burnBy=' + $(this).val() + "&zentaoProject=" + zentaoProject);
    });
