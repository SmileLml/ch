<?php
/**
 * The task group view file of chproject module of ZenTaoPMS.
 *
...
<div id="mainMenu" class="clearfix table-row">
  <div class="btn-toolbar pull-left">
    ...
    <?php echo html::a($this->createLink('chproject', 'taskeffort', "projectID=$projectID&group=$groupBy&type=$weekend&intanceProjectID=$intanceProjectID"), $lang->execution->$weekend, '', "class='btn btn-link'");?>
    <div class='btn-group'>
      ...
      <ul class='dropdown-menu' style='max-height:240px; max-width: 300px; overflow-y:auto'>
        <?php
          ...
          echo "<li $class>" . html::a($this->createLink('chproject', 'taskeffort', "projectID=$projectID&groupBy=$groupBy&type=$type&intanceProjectID=0"), $lang->chproject->allProject) . "</li>";
          foreach($intanceProjectPairs as $key => $intanceProjectName)
          {
              $class = $intanceProjectID == $key ? 'class="active"' : '';
              echo "<li $class>" . html::a($this->createLink('chproject', 'taskeffort', "projectID=$projectID&groupBy=$groupBy&type=$type&intanceProjectID=$key", ""), $intanceProjectName, '', "title='{$intanceProjectName}' class='text-ellipsis'") . "</li>";
          }
        ?>
      </ul>
    </div>
...
<div id='tasksTable' class='main-table load-indicator loading' data-ride='table' data-checkable='false' data-group='true' data-hot='true'>
  ...
  <?php else:?>
  <?php $projectBegin = helper::isZeroDate($project->realBegan) ? $project->begin : $project->realBegan;?>
  <?php if((time() - strtotime($projectBegin)) < 0):?>
  <div class="table-empty-tip">
    <p>
      ...
      <div id='headerColGroup'>
        <div id='groupMenu' class="dropdown">
          ...
          <ul class="dropdown-menu">
            ...
            echo "<li $active>"; common::printLink('chproject', 'taskeffort', "projectID=$projectID&groupBy=$key&type=$type&intanceProjectID=$intanceProjectID", $value); echo '</li>';
            ?>
            <?php endforeach;?>
          </ul>
        </div>
