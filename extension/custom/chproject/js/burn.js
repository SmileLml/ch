$(function()
{
    ...
    {
        location.href = createLink('chproject', 'burn', 'projectID=' + projectID + '&type=' + type + '&interval=' + $(this).val());
    });

    $('#weekend').click(function()
    {
        var browseType = type.match('noweekend') ? type.replace('noweekend', 'withweekend') : type.replace('withweekend', 'noweekend');
        location.href = createLink('chproject', 'burn', 'projectID=' + projectID + '&type=' + browseType + '&interval=' + interval);
    });
    ...
    $("#projectBy").change(function()
    {
        var browseType = type.match('nodelay') ? type.replace('nodelay', 'withdelay') : type.replace('withdelay', 'nodelay');
        location.href = createLink('chproject', 'burn', 'projectID=' + projectID + '&type=' + browseType + '&interval=' + interval + "&burnBy=" + burnBy + "&zentaoProject=" + $(this).val());
    });
})
...
function downloadBurn()
{
    ...
    setTimeout(function()
    {
        ...
        var filename = $("#projectBy").find('option:selected').text() + "-" + executionName + '.' + type;
