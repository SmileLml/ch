...
{
    var link = createLink('chproject', 'kanban', "executionID=" + executionID + '&type=' + view);
    location.href = link;
}
...
function createColumnMenu(options)
{
	...
	var items = [];
	if(priv.canEditName) items.push({label: executionLang.editName, url: $.createLink('kanban', 'setColumn', 'col=' + col.id + '&executionID=' + executionID + '&from=chproject'), className: 'iframe', attrs: {'data-width': '500px'}})
	if(priv.canSetWIP) items.push({label: executionLang.setWIP, url: $.createLink('kanban', 'setWIP', 'col=' + col.id + '&executionID=' + executionID + '&from=chproject'), className: 'iframe', attrs: {'data-width': '500px'}})
...
function createColumnCreateMenu(options)
{
    ...
    {
        if(priv.canCreateStory) items.push({label: storyLang.create, url: $.createLink('story', 'create', 'productID=' + productID + '&branch=0&moduleID=0&storyID=0&objectID=0&bugID=0&planID=0&todoID=0&extra=&storyType=story&chproject=' + executionID, '', true), className: 'iframe', attrs: {'data-width': '80%'}});
    }
    else if(col.laneType == 'bug')
    {
        if(priv.canCreateBug) items.push({label: bugLang.create, url: $.createLink('bug', 'create', 'productID=0&moduleID=0&extras=&chprojectID=' + executionID, '', true), className: 'iframe', attrs: {'data-width': '80%'}});
    }
    else
    {
        if(priv.canCreateTask) items.push({label: taskLang.create, url: $.createLink('task', 'create', 'executionID=0&storyID=0&moduleID=0&taskID=0&todoID=0&extra=&bugID=0&chprojectID=' + executionID, '', true), className: 'iframe', attrs: {'data-width': '80%'}});
    }
    return items;
}
...
function resizeKanbanContainer()
{
    ...
    var maxHeight = window.innerHeight - 98 - 65;
...
function changeKanbanScaleSize(newScaleSize)
{
    ...
    $.zui.store.set('chprojectKanbanScaleSize', newScaleSize);
...
function handleSortCards(event)
{
    ...
    $.getJSON(url, function(response)
    {
        ...
        {
            $.get(createLink('execution', 'ajaxUpdateKanban', "executionID=" + executionID + "&entertime=0&browseType=" + browseType + "&groupBy=" + groupBy + '&from=chproject' + '&searchValue=' + searchValue + '&orderBy=' + orderBy), function(data)
            {
...
$(function()
{
    ...
    window.kanbanScaleSize = +$.zui.store.get('chprojectKanbanScaleSize', 1);
    ...
    {
        $.get(createLink('execution', 'ajaxUpdateKanban', "executionID=" + executionID + "&entertime=" + entertime + "&browseType=" + browseType + "&groupBy=" + groupBy + '&from=chproject' + '&searchValue=' + searchValue + '&orderBy=' + orderBy), function(data)
        {
...
$('#type').change(function()
{
    ...
    var link = createLink('chproject', 'kanban', "executionID=" + executionID + '&type=' + type);
    location.href = link;
});
...
$('.c-group').change(function()
{
    ...
    var type  = $('#type').val();
    var group = $('#group').val();
    var link  = createLink('chproject', 'kanban', 'executionID=' + executionID + '&type=' + type + '&orderBy=order_asc' + '&groupBy=' + group);
    location.href = link;
});
...
function searchCards(value, order = '')
{
    ...
    $.get(createLink('execution', 'ajaxUpdateKanban', "executionID=" + executionID + "&entertime=0&browseType=" + browseType + "&groupBy=" + groupBy + '&from=chproject&searchValue=' + value + '&orderBy=' + orderBy), function(data)
    {
        lastUpdateData = data;
