      ...
      <tr>
        ...
        <td class='c-title text-left' title='<?php echo $review->title?>'>
          ...
          else
          {
              ...
              if($type == 'review') $class = 'data-app=my';
              $titleHtml = html::a($this->createLink($type, 'view', "objectID=$review->id", 'html', $type == 'review' ? false : true), $review->title, '', $class);
          }
          echo $titleHtml;
          ?>
        </td>
        ...
        <?php endif;?>
        <?php if($review->type == 'business'):?>
        <td class='c-status'><?php echo zget($lang->business->actionList, $review->action, '');?></td>
        <?php elseif($review->type == 'projectapproval'):?>
        <td class='c-status'><?php echo zget($lang->projectapproval->actionList, $review->action, '');?></td>
        <?php else:?>
        ...
        <td class='c-actions text-left'>
          ...
          elseif($module == 'projectapproval' && $review->action == 'approvalreview5')
          {
              common::printLink($module, $review->action, $params, $reviewIcon, '', "class='btn' title='{$lang->review->common}' target='_blank'");
          }
          ...
          elseif($module == 'business' && $review->action == 'prdreview')
          {
              common::printLink($module, $review->action, $params, $reviewIcon, '', "class='btn' data-toggle='ajax' title='{$lang->review->common}'", true, true);
          }
          elseif(!in_array($module, array('demand', 'story', 'testcase', 'feedback')))
          {
              common::printLink($module, $review->action, $params, $reviewIcon, '', "class='btn iframe' title='{$lang->review->common}'", true, true);
          }
          else
          {
