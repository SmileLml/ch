    ...
    <form method='post' target='hiddenwin' id='dataform' class='main-form form-ajax' enctype='multipart/form-data'>
      <table class='table table-form'>
        ...
        <tr>
          ...
          if($mode == 'new')
          {
              echo html::select('dept', $depts, '', "class='form-control chosen'");
          }
          else
          {
            echo html::select('dept', $depts, '', "$readonly class='form-control chosen'");
          }
          ?>
          </td>
        </tr>
...
<script>
  $(document).ready(function()
  {
    ...
    function selectUser(deptDom)
    {
      var dept = $(deptDom).val();
      $.get(createLink('my', 'ajaxGetDeptAllUsers', 'deptID=' + dept), function(users)
      {
        $('#users').data('zui.picker').setValue(users)
      }, 'json')
    }
  })
</script>
