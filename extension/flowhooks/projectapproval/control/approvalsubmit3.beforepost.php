...
if(empty($approvalReviewers)) return $this->send(array('result' => 'fail', 'message' => $this->lang->flow->leastOneApprover));
...
if($isNotReviewer) return $this->send(array('result' => 'fail', 'message' => $this->lang->flow->leastOneApprover));
...
if(isset($_POST['children']['sub_projectbusiness']['business']))
{
    ...
    $costType = array_search('itPlanInto', $_POST['children']['sub_projectcost']['costType']);
    if(!$costType) return $this->send(array('result' => 'fail', 'message' => array('sub_projectcost' => $this->lang->flow->needitPlanInto)));
    ...
    $allDevelopmentBudget = 0;
    foreach($_POST['children']['sub_projectbusiness']['developmentBudget'] as $budget) $allDevelopmentBudget += $budget;

    if($allDevelopmentBudget > $itPlanIntoCostBudget) return $this->send(array('result' => 'fail', 'message' => array(('childrensub_projectcostcostBudget' . $costType) => $this->lang->flow->overSetBudget)));
