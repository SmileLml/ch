...
if($project->status == 'approvedProject')
{
    ...
    {
        $actionID = $this->action->create('business', $tempBusinessID, 'projectreview2');
        $change = array();
        $change[] = array('field' => 'status', 'old' => 'projecting', 'new' => 'approvedProject');
        $this->action->logHistory($actionID, $change);
    }
    
    $this->loadModel('projectapproval')->updateProjectApproval($dataID, $projectBusinesses, 'projectreview2');
